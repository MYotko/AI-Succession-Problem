...
import defense_heldout_run_a3s_executor as ex
...
def attack_validity():
    ...
    with (OUT/(P+'steps.csv')).open(encoding='utf-8',newline='') as stream:
        for row in csv.DictReader(stream):
            if row['defense_arm']!='OFF' or row['step']!='149':continue
            groups[row['arm']].append({'seed':int(row['seed']),'resilience_stock':float(row['resilience_stock'])})
    expected=list(range(1835087900,1835087920))
    for rows in groups.values():assert sorted(r['seed'] for r in rows)==expected,'Missing OFF step-149 stock'
    value=paired_difference(groups['C'],groups['A3'],'resilience_stock')
    control_median=statistics.median(r['resilience_stock'] for r in groups['C'])
    attacked_median=statistics.median(r['resilience_stock'] for r in groups['A3'])
    margin=control_median-attacked_median;t=value['t_statistic']
    median_met=margin>=0.10;t_met=False if t is None else t>=2.0
    result={'contrast':'C OFF minus A3 OFF','step':149,'control_median_stock':control_median,
            'A3_median_stock':attacked_median,'median_control_minus_A3':margin,
            'median_margin_threshold':0.10,'median_threshold_met':median_met,
            'paired':value,'paired_t_threshold':2.0,'paired_t_threshold_met':t_met,
            'attack_validity_met':median_met and t_met,'void_attack':not(median_met and t_met),
            'defended_comparisons_read_before_validity':False,'per_run_stock':groups,'bias':BIAS,
            'computed_utc':ex.now()}
...
def observations():
    ...
    for c in summaries:
        ...
        result.append({'arm':c['arm'],'defense_arm':c['defense_arm'],'seed':c['seed'],
                       ...
                       'resilience_stock_step_49':baseline,
                       'resilience_stock_step_149':rows[149]['resilience_stock'] if len(rows)>149 else None,
                       'depletion_step':depletion,
    ...
    assert len(result)==80 and not grouped
    return result,summaries

def calculate(runs,validity):
    ...
    for arm in ex.ARMS:
        ...
        r['Y3'][arm]={'harm_direction':DIRECTION['Y3'],
                     ...
                     'undefined_pair_seeds':[s for s in range(1835087900,1835087920) if s not in defined],
                     'bias':BIAS}
    ...
    predictions={}
    predictions['1']={'median_control_minus_A3':validity['median_control_minus_A3'],
                      'threshold':0.10,'comparison':'>=',
                      'meets_numeric_threshold':validity['median_threshold_met'],
                      'attack_validity_met':validity['attack_validity_met'],'bias':BIAS}
    count=r['Y4']['A3']['cells']['OFF']['no_alarm_before_150_count']
    predictions['2']={'number':count,'threshold':15,'comparison':'>=',
                      'meets_numeric_threshold':count>=15,'bias':BIAS}
    attacked=[x for x in runs if x['arm']=='A3' and x['defense_arm']=='OFF']
    control=[x for x in runs if x['arm']=='C' and x['defense_arm']=='OFF']
    defined={x['seed'] for x in attacked if x['shock_loss'] is not None}&{x['seed'] for x in control if x['shock_loss'] is not None}
    value=pair([x for x in attacked if x['seed'] in defined],
               [x for x in control if x['seed'] in defined],'shock_loss')
    t=value['t_statistic']
    predictions['3']={'contrast':'A3 OFF minus C OFF on shock loss','paired':value,
                      'undefined_pair_count':20-len(defined),
                      'undefined_pair_seeds':[s for s in range(1835087900,1835087920) if s not in defined],
                      't_threshold':2.0,'comparison':'>=',
                      'meets_numeric_threshold':None if t is None else value['mean_difference']>0 and t>=2.0,
                      'bias':BIAS}
    value=r['Y1']['A3']['paired'];t=value['t_statistic']
    predictions['4']={'paired':value,'absolute_t_threshold':2.0,
                      'meets_numeric_threshold':None if t is None else abs(t)<=2.0,'bias':BIAS}
    return r,predictions

def report_complete(results,runs):
    validity=results['attack_validity'];p=validity['paired']
    lines=['# Held-out attack, stage A3','','## Attack validity','',
       '| C OFF median stock | A3 OFF median stock | Median difference | Margin threshold | Pairs | Mean C OFF minus A3 OFF | Paired standard error | t | t threshold | Basis |',
       '| ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |',
       '| '+' | '.join(map(str,[validity['control_median_stock'],validity['A3_median_stock'],
            validity['median_control_minus_A3'],0.10,p['n_pairs'],p['mean_difference'],
            p['paired_standard_error'],'undefined' if p['t_statistic'] is None else p['t_statistic'],2.0,BIAS]))+' |','',
       'Median threshold met: '+str(validity['median_threshold_met'])+'. Paired t threshold met: '+str(validity['paired_t_threshold_met'])+'.',
       'Validity was computed and recorded before any defended outcome comparison was read.']
    if validity['void_attack']:
        lines += ['A3 is VOID. Its defended-arm numbers are reported below and may not be cited as evidence about the defense.']
    else:
        lines += ['The declared attack-validity thresholds are met.']
    lines += ['','This is gate 2 of the promotion plan. The attacks were specified before any run.',
       ...
       'Amendment 2 adds A3, which moves the entire resilience share to institutional capacity. Stage A artifacts remain committed under their original plan.','']
...
def finalize(results,lines,summaries):
    ...
    if (OUT/(P+'gates.json')).exists():
        ...
        for key,value in evidence.items():
            if key in ('source_pins_start','8_source_pins_end','runtime','2_attack_arithmetic','2_attack_effect'):continue
...
def main():
    if '--halt' in sys.argv:
        ...
        results={'status':'HALTED','runs_planned':80,
        ...
        lines=['# Held-out attack, stage A3: halt','',
               ...
               'Amendment 2 adds stage A3 with a full-share drain and a separate attack-validity comparison.','',
        ...
        for name in ('constants_gate.json','no_oracle_gate.json','gates.json','attack_arithmetic_gate.json','attack_effect_gate.json'):
    ...
    registered,predictions=calculate(runs,validity)
    results={'status':'COMPLETE','runs':len(runs),'registered':registered,
             'sign_fixture':fixture,'attack_validity':validity,'prediction_numeric_checks':predictions,
