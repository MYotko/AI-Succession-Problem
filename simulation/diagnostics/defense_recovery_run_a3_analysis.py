...
import defense_recovery_run_a3_executor as ex
...
def attack_validity():
    ...
    for rows in groups.values():
        assert sorted(r['seed'] for r in rows)==list(range(1835088000,1835088020))
...
def observations():
    ...
    for completion in summaries:
        ...
        runs.append(row)
    assert len(runs)==300 and not grouped
...
def calculate(runs):
    ...
    for arm in ex.ARMS:
        ...
        for cell in cells.values():
            assert sorted(r['seed'] for r in cell)==list(range(1835088000,1835088020))
        for q,field in [('R1','g_steps'),('R2','final_population')]:
            contrasts={}
...
def report_complete(results,runs):
    lines=['# In-loop drift defense, gate 3: recovery evaluation, attempt 3','',
...
def main():
    if '--halt' in sys.argv:
        ...
        results={'status':'HALTED','runs_planned':300,
        ...
        lines=['# Recovery evaluation, attempt 3: halt','',
