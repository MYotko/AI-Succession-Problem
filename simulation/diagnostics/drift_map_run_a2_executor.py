...
def allowed(path):
    p=Path(path).resolve()
    return p==NULL or(p.parent==OUT and p.name.startswith('drift_map_run_a2_'))
...
def pins():
    plan=read('drift_map_run_a2_plan.json');records=[]
    ...
    if not all(r['passed']for r in records):
        write('drift_map_run_a2_source_pin_failure_'+str(os.getpid())+'.json',{'utc':now(),'pins':records})
...
def run_model(job,arm,seed,gate=False,attempt=1):
    ...
    if arm=='H' and model.attack_vector_v2 is not None:raise RuntimeError('Honest model has an attack vector')
    if not gate:
        honest_seed={s:s for s in plan['seeds']}[seed]
        if seed!=honest_seed:raise RuntimeError('Attack seed differs from paired honest seed')
        if arm.startswith('M'):
            m=plan['arms'][arm]
            adapter._apply_sub_threshold_drift=lambda model,action,step:multiplied(model,action,step,m)
        elif arm.startswith('R'):
            d=plan['arms'][arm]
            adapter._apply_sub_threshold_drift=lambda model,action,step:reallocated(model,action,step,d)
    horizon=60 if gate and arm=='H' else 300
    initial_counter=int(metrics.H_N_SHAPE_FALLBACK_COUNT)
    runtime=metadata(np)
    write(P+job+'_initial.json',{'job':job,'arm':arm,'seed':seed,'configuration':configuration,
          'attack_vector_v2':model.attack_vector_v2,'shape_fallback_counter_before_step_0':initial_counter,
          'runtime':runtime,'identity':identity(),'started_utc':started,'attempt':attempt})
    logname=P+job+'_steps_attempt'+str(attempt)+'.csv.partial'
    rows=0;active=0;modified=0;alive=True;counter_step0=None
    with (OUT/logname).open('x',encoding='utf-8',newline='') as f:
        ...
        for step in range(horizon):
            ...
            current=int(metrics.H_N_SHAPE_FALLBACK_COUNT)
            if step==0:counter_step0=current
            if step>0 and current-counter_step0!=0:
                write(P+job+'_fallback_failure.json',{'step':step,'before_step_0':initial_counter,
                      'end_step_0':counter_step0,'current':current,'increase_after_step_0':current-counter_step0})
                raise RuntimeError('Continuous check: shape fallback increased after step 0')
            if writer is None:
            ...
            if step%25==0:write(P+job+'_progress.json',{'job':job,'arm':arm,'seed':seed,'completed_steps':rows,'target':horizon,'utc':now()})
            if not alive:break
        f.flush();os.fsync(f.fileno())
    final_counter=int(metrics.H_N_SHAPE_FALLBACK_COUNT)
    delta0=counter_step0-initial_counter;delta_after=final_counter-counter_step0
    if delta_after!=0:raise RuntimeError('Continuous check: shape fallback increased after step 0')
    if gate and rows!=horizon:raise RuntimeError('Gate probe did not complete required steps')
    ...
    result={'status':'COMPLETE','job':job,'arm':arm,'seed':seed,'paired_honest_seed':seed,
            'configuration':configuration,'identity':identity(),'attempt':attempt,'steps_completed':rows,
            'extinct':not bool(alive),'shape_fallback_increase_step_0':delta0,
            'shape_fallback_increase_after_step_0':delta_after,'shape_counter_before_step_0':initial_counter,
            'shape_counter_end_step_0':counter_step0,'shape_counter_end':final_counter,
...
def worker(args):
    try:
        if args.job=='gate_wrapper':wrapper_gate()
            ...
            run_model(args.job,'H' if args.job=='gate_honest' else 'GATE_ATTACK',1835086199,gate=True)
...
def compare_gates():
    ...
    first=None;checks=0
    if factory['configuration']!=common['configuration']:first={'kind':'configuration','step':None}
    a=factory['datacollector'];b=common['datacollector']
    if first is None and list(a)!=list(b):first={'kind':'datacollector keys','step':None,'factory':list(a),'common':list(b)}
    if first is None:
        for key in a:
            if len(a[key])!=len(b[key]):first={'field':key,'step':None,'kind':'length'};break
            for step,(x,y) in enumerate(zip(a[key],b[key])):
                checks+=1
                if x!=y:first={'field':key,'step':step,'factory':x,'common':y};break
    ...
    if first is None:
        for step,(x,y) in enumerate(zip(ra,rb)):
            for key in x:
                checks+=1
                if x[key]!=y[key]:first={'field':key,'step':step,'factory':x[key],'common':y[key]};break
            if first is not None:break
    honest=read(P+'gate_honest_result.json');wrapper=read(P+'gate_wrapper_result.json')
    results={'gate_2':{'passed':True,'pins':pins()},
             'gate_3':{'passed':first is None and len(ra)==len(rb)==300,'configuration_equal':factory['configuration']==common['configuration'],
                       'datacollector_fields':len(a),'recorder_fields':len(ra[0]),'steps_compared':len(ra),'field_value_comparisons':checks,'first_difference':first},
             'gate_4':{'passed':wrapper['passed'],'comparisons':wrapper['comparisons'],'maximum_key_difference':wrapper['maximum_key_difference']},
             ...
             'amendment_1':{j:{'step_0':r['shape_fallback_increase_step_0'],'after_step_0':r['shape_fallback_increase_after_step_0']} for j,r in [('factory',factory),('common',common),('honest',honest)]},
             'completed_utc':now(),'identity':identity()}
    results['passed']=all(results['gate_'+str(i)]['passed'] for i in range(2,7)) and all(v['after_step_0']==0 for v in results['amendment_1'].values())
    write(P+'gates.json',results)
    if not results['passed']:raise RuntimeError('T1 gate failed: '+json.dumps(results))
    return results

def gates():
    scheduler_checks();pins();check_stop()
    jobs=['gate_wrapper','gate_factory','gate_common','gate_honest']
    ...
    try:
        ...
        write(P+'gate_execution.json',{'maximum_concurrent_processes':maximum,'simulation_workers':3,'completed_jobs':done,'utc':now()})
        print(json.dumps({'T1':'PASS','gates':result}),flush=True)
    except BaseException as e:
        mark_halt('gates',e)
...
def completed(job,arm,seed):
    ...
    if r['configuration']!=expected_config(seed,arm=='H') or r['shape_fallback_increase_after_step_0']!=0:
