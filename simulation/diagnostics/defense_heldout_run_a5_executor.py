"""Execute the held-out attack evaluation under the attempt 5 write guard."""
import sys
...
def allowed(path):
    p=Path(path).resolve()
    return p==NULL or(p.parent==OUT and p.name.startswith('defense_heldout_run_a5_'))
...
def attack_arithmetic_gate():
    ...
    for arm,rate in (('A1-low',0.002),('A1-high',0.006)):
        wrapper=Attack(arm).wrap(production)
        for index in range(24):
            resilience=(0.0,0.0005,0.02,0.006)[index%4]
            weights=(0.1,0.2,0.25,0.15,0.3)
            offset=index%5
            weights=weights[offset:]+weights[:offset]
            ...
            action=adapter._normalize_action(action)
            original=deepcopy(action)
            moved=min(float(action['x_resilience']),rate)
            expected=deepcopy(action)
            expected['x_resilience']-=moved
            ...
            category='zero' if action['x_resilience']==0.0 else ('below' if action['x_resilience']<rate else 'at_or_above')
            cases.append({'arm':arm,'rate':rate,'case':index,'resilience_category':category,
                          ...
                          'passed':passed})
    result={'passed':all(c['passed'] for c in cases),'cases_per_rate':24,
...
def attack_effect_gate():
    ...
    control={r['step']:r for r in log_rows(a['raw_log'])}
    attacked=log_rows(b['raw_log'])
    comparisons=[]
    for row in attacked:
        step=row['step']
        if step<=50 or step not in control:continue
        delta=control[step]['x_resilience']-row['x_resilience']
        comparisons.append({'step':step,'control_resilience_share':control[step]['x_resilience'],
                            'attacked_resilience_share':row['x_resilience'],
                            'control_minus_attacked':delta,'lower_with_tolerance':delta>1e-9})
    below=sum(c['lower_with_tolerance'] for c in comparisons)
    result={'passed':below>=100,'steps_strictly_after_50_with_lower_resilience_share':below,
            'required_count':100,'computed_float_absolute_tolerance':1e-9,
            'comparison':'control resilience minus attacked resilience greater than 1e-9',
            'comparisons':comparisons}
    write(P+'attack_effect_gate.json',result)
    assert result['passed'],'T1 gate 2: trajectory resilience share'
    return result
...
def gates():
    ...
    evidence['2_attack_arithmetic']=attack_arithmetic_gate()
