...
import detector_run_r3_eval_executor as h
import numpy as np
...
def finalize():
    ...
    if(h.OUT/name).exists():
        ...
        doc+=[f"Constructor comparison: {g['steps_compared']} completed steps, {g['recorder_fields']} recorded fields, {g['field_value_comparisons']} field comparisons; first difference: {g['first_difference']}. Factory and common end reasons: {g['factory_end_reason']}, {g['common_end_reason']}.",'',
              ...
              'Every channel constant, allocation median, threshold, and hazard parameter is checked bitwise against the committed stage A and earlier files. No constant was recomputed. Details are in detector_run_r3_eval_constants_gate.json.','',
    ...
    doc+=['Exploratory descriptive recording required by Amendment 2:','',
          ...
          'Operational retry implementation: detector_run_r3_eval_executor.py, LF-normalized SHA256 '+h.sha(h.lf((h.OUT/(h.P+'executor.py')).read_bytes()))+'.','',
