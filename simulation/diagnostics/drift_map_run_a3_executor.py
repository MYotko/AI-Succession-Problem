...
def allowed(path):
    p=Path(path).resolve()
    return p==NULL or(p.parent==OUT and p.name.startswith('drift_map_run_a3_'))
...
def pins():
    plan=read('drift_map_run_a3_plan.json');records=[]
    ...
    note_actual=sha(lf((ROOT/plan['design_note']).read_bytes()))
    if not all(r['passed'] for r in records) or note_actual!=plan['design_sha256_lf']:
        write(P+'source_pin_failure_'+str(os.getpid())+'.json',{'utc':now(),'pins':records,'design_sha256_lf':note_actual})
...
def run_model(job,arm,seed,gate=False,attempt=1):
    ...
    if not gate:
        ...
        if model.random_seed!=honest_seed or configuration['random_seed']!=honest_seed:
            raise RuntimeError('Attack seed differs from paired honest seed')
        if arm.startswith('M'):
            m=plan['arms'][arm]
    ...
    if gate and arm=='H' and rows!=60:raise RuntimeError('Gate 5 and 6 probe did not complete 60 steps')
...
def compare_gates():
    ...
    results={'gate_2':{'passed':True,'pins':pins()},
             'gate_3':{'passed':first is None,'seed':1835086199,
                       ...
                       'field_value_comparisons':checks,'null_encoding':'null','first_difference':first},
    ...
    results['passed']=all(results['gate_'+str(i)]['passed'] for i in range(2,7))
