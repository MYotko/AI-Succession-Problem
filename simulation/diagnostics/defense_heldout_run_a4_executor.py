"""Execute the held-out attack evaluation under the attempt 4 write guard."""
import sys
...
def allowed(path):
    p=Path(path).resolve()
    return p==NULL or(p.parent==OUT and p.name.startswith('defense_heldout_run_a4_'))
...
ARMS=('A1-high','A1-low','A2','C')
DEFENSE_ARMS=('GRADED','OFF')
...
class Attack:
    ...
    def wrap(self,production):
        if self.arm=='C':return production
        def wrapped(model,ai,action,step,actor_role='incumbent'):
            result=production(model,ai,action,step,actor_role=actor_role)
            if actor_role!='incumbent':return result
            import attack_adapter_v2 as adapter
            returned=result;kind='inactive';moved=0.0;normalization={}
            if self.arm=='A2' and 50<=step<=52:
                returned=adapter._hostile_action(result);kind='burst'
            elif step>=50 and (self.arm!='A2' or step>=53):
                rate=0.002 if self.arm=='A1-low' else 0.006
                returned=deepcopy(result)
                moved=min(float(returned['x_resilience']),rate)
                returned['x_resilience']-=moved
                returned['x_institutional_capacity']+=moved
                before=deepcopy(returned)
                returned=adapter._normalize_action(returned);kind='siphon'
...
def run_model(job,arm,defense_arm,seed,gate=False,force_normal=False,attempt=1):
    import numpy as np
    import model as model_module,metrics
    check_stop();start_pins=pins();clock=time.perf_counter();started=now()
    channels,medians,_,_=constants()
    model=constructor(seed,True)
    assert model.config==expected_config(seed,True)
    assert model.config['shock_step']==150
    assert float(model.config['shock_magnitude']).hex()==float(0.60).hex()
    assert model.attack_step==50 and model.attack_vector_v2 is None and not model.cop_cusum_drift
    ...
    with (OUT/rawname).open('x',encoding='utf-8',newline='') as handle:
        ...
        def observed_step():
            ...
            observed=record(model,'H',seed,step,np)
    ...
    audit_value={'job':job,'arm':arm,'defense_arm':defense_arm,'seed':seed,
                 ...
                 'binding_restored_by_identity':True,'step_observer_restored_by_identity':True}
...
def validate_completion(job,gate=False):
    ...
    assert result['configuration']==expected_config(job['seed'],True)
    ...
    assert result['binding_restored_by_identity'] and result['step_observer_restored_by_identity']
...
def attack_effect_gate():
    a=read(P+'gate_C_OFF_result.json');b=read(P+'gate_A1_high_OFF_result.json')
    control=log_rows(a['raw_log']);attacked=log_rows(b['raw_log'])
    comparisons=[];below=0;sum_failures=[];difference=None
    rate=0.006;tolerance=1e-9
    for row in attacked:
        step=row['step'];total=sum(row[k] for k in FIELDS)
        if abs(total-1.0)>1e-12:sum_failures.append({'step':step,'sum':total})
        if step>=len(control):
            difference=difference or {'step':step,'reason':'control has no completed corresponding step'}
            continue
        c=control[step]
        lower=c['x_resilience']-row['x_resilience']>tolerance
        if step>50:below+=int(lower)
        for field in FIELDS:
            if field=='x_resilience':continue
            delta=row[field]-c[field]
            comparison=delta if field=='x_institutional_capacity' else abs(delta)
            bound=rate+tolerance
            comparisons.append({'step':step,'field':field,'signed_control_difference':delta,
                                'comparison_value':comparison,'bound':bound})
            if comparison>bound and difference is None:
                difference={'step':step,'field':field,'control':c[field],'attacked':row[field],
                            'signed_difference':delta,'comparison_value':comparison,
                            'rate':rate,'absolute_tolerance':tolerance,'bound':bound}
    result={'passed':below>=100 and not sum_failures and difference is None,
            'steps_strictly_after_50_with_lower_resilience_share':below,
            'required_count':100,'share_sum_failures':sum_failures,
            'computed_float_absolute_tolerance':tolerance,'share_sum_absolute_tolerance':1e-12,
            'lower_resilience_comparison':'control minus attacked greater than 1e-9',
            'institutional_comparison':'signed increase at most rate plus 1e-9',
            'other_share_comparison':'absolute difference at most rate plus 1e-9 on every step',
            'first_other_share_bound_violation':difference,'comparisons':comparisons}
    write(P+'attack_effect_gate.json',result)
    if not result['passed']:raise RuntimeError('T1 gate 2 failed; see '+P+'attack_effect_gate.json')
    return result
...
def gates():
    ...
    for field,value in expected.items():
        passed=float(CONSENSUS[field]).hex()==float(value).hex()
        checks.append({'quantity':'consensus.'+field,'actual':CONSENSUS[field],'expected':value,
                       'binary64_hex':float(value).hex(),'passed':passed})
    assert all(c['passed'] for c in checks)
    evidence['8_constants']={'passed':True,'checks':checks,'channels':channels,'medians':medians}
    write(P+'constants_gate.json',evidence['8_constants'])
    evidence['6_no_oracle']=no_oracle_gate()
    jobs=read(P+'plan.json')['gate_jobs']
    dispatch(jobs[:2],'gate_initial')
    evidence['1_pre_onset']=compare_fields('gate_C_OFF','gate_A1_high_OFF',50)
    write(P+'gates.json',{'passed':False,'status':'IN_PROGRESS',**evidence})
    evidence['2_attack_effect']=attack_effect_gate()
    dispatch(jobs[2:],'gate_remaining')
    burst=read(P+'gate_A2_OFF_result.json')
    rows=log_rows(burst['raw_log']);audit_value=read(burst['audit_file'])
    burst_cases=[]
    for step in (50,51,52):
        call=next(c for c in audit_value['attack_calls'] if c['step']==step)
        actual={k:rows[step][k] for k in CONSENSUS}
        expected={k:call['returned_action'][k] for k in CONSENSUS}
        burst_cases.append({'step':step,'actual':actual,'expected_production_hostile_action':expected,'absolute_tolerance':1e-9,'passed':all(abs(float(actual[k])-float(expected[k]))<=1e-9 for k in expected) and call['kind']=='burst'})
    evidence['3_burst']={'passed':all(c['passed'] for c in burst_cases),'cases':burst_cases}
    write(P+'burst_gate.json',evidence['3_burst'])
    assert evidence['3_burst']['passed'],'T1 gate 3: burst identity'
    evidence['4_composition']=[compare_fields('gate_C_OFF','gate_C_NORMAL'),
                               compare_fields('gate_A1_high_OFF','gate_A1_high_NORMAL')]
    evidence['5_online_equivalence']={}
    for name in ('gate_A1_high_GRADED','gate_A2_GRADED'):
        result=read(P+name+'_result.json')
        evidence['5_online_equivalence'][name]=result['offline_equivalence']
    result=read(P+'gate_A2_GRADED_result.json')
    evidence['7_incumbent_calls']={'passed':all(n==1 for n in result['incumbent_call_counts'].values()),
                                   'per_step_counts':result['incumbent_call_counts']}
    assert evidence['7_incumbent_calls']['passed']
    evidence['9_source_pins_end']=pins(full=True)
...
def scheduler_checks():
    measured=[slots(*x) for x in [('normal',0,160),('work',15,10),('work',13,10),('work',12,10),('work',11,10),('normal',12,10),('normal',0,2)]]
    ...
    assert len(pending)==158
    assert all(j['seed'] in range(1835087800,1835087820) for j in pending)
    ...
    result={'passed':True,'normal_cap':15,'work_cap':12,'slot_measurements':measured,
            'synthetic_resume_preserved':2,'synthetic_resume_pending':158,
...
def main():
    ...
    try:
        ...
        print(json.dumps({'status':'COMPLETE','runs':160,'report':P+'report.md','results':P+'results.json','manifest':P+'manifest.json'}),flush=True)
