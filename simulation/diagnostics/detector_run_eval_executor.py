...
def allowed(path):
    p=Path(path).resolve()
    return p==NULL or(p.parent==OUT and p.name.startswith('detector_run_eval_'))
...
def pins():
    plan=read('detector_run_eval_plan.json');records=[]
...
def run_model(job,arm,seed,gate=False,attempt=1):
    ...
    if gate:result['datacollector']=model.datacollector
        ...
        csv_output(hb,output['heartbeats'],['record_type','step','heartbeat_counter','S_H','S_g','S_L'])
        csv_output(alarms,output['alarm_records'],['record_type','step','channel','statistic_before_reset','statistic_after_reset','operational_channel'])
        result['detector_logs']={name:sha((OUT/name).read_bytes()) for name in (hb,alarms)}
...
def compare_gates():
    ...
    results['passed']=all(results['gate_'+str(i)]['passed'] for i in range(2,7)) and results['recorder_conformance']['passed'] and results['constants_conformance']['passed']
...
def completed(job,arm,seed):
    ...
    if r['heartbeat_count']!=r['steps_completed']:raise RuntimeError('Completed heartbeat count mismatch: '+job)
...
def constants():
    ...
    item=next(x for x in read(P+'plan.json')['pins'] if x['path']==path)
    raw=git('cat-file','blob','HEAD:'+path)
    if sha(lf(raw))!=item['expected_sha256_lf']:raise RuntimeError('Committed constants pin changed')
    source=json.loads(raw)['channels']
    supplied={channel:{key:source[channel][key] for key in ('reference','allowance','threshold')} for channel in ('entropy','g','L')}
    import struct
    bits=lambda value:struct.pack('!d',value).hex()
    checks=[{'channel':c,'field':k,'committed_bits':bits(source[c][k]),'passed_bits':bits(supplied[c][k]),
             'passed':bits(source[c][k])==bits(supplied[c][k])} for c in supplied for k in supplied[c]]
    if not all(x['passed'] for x in checks):raise RuntimeError('Constants conformance failed')
    return supplied,checks

def constants_gate():
    supplied,checks=constants()
    result={'passed':all(x['passed'] for x in checks),'checks':checks,'values_passed':supplied,'identity':identity(),'utc':now()}
    write(P+'constants_gate.json',result)
    return result
...
def evaluate_records(rows):
    ...
    if len(output['heartbeats'])!=len(rows):raise RuntimeError('Heartbeat count differs from completed steps')
    ...
    stars=read(P+'plan.json')['g_stars']
    crossings={label:next((r['step'] for r in rows if r['step']>=50 and r['g']>=value),None) for label,value in stars.items()}
    return output,{'heartbeat_count':len(output['heartbeats']),'alarm_records':output['alarm_records'],
                   't_star':crossings,'detection_alarm_step':detection,'first_detection_steps':first,
                   'constants_passed':supplied,'constants_bitwise_checks':checks}
