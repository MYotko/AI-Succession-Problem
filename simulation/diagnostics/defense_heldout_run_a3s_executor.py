"""Execute the held-out attack evaluation under the stage A3 write guard."""
import sys
...
def allowed(path):
    p=Path(path).resolve()
    return p==NULL or(p.parent==OUT and p.name.startswith('defense_heldout_run_a3s_'))
...
def attack_effect_gate():
    a=read(P+'gate_C_OFF_result.json');b=read(P+'gate_A3_OFF_result.json')
...
def gates():
    ...
    assert all(c['passed'] for c in checks)
    evidence['7_constants']={'passed':True,'checks':checks,'channels':channels,'medians':medians}
    write(P+'constants_gate.json',evidence['7_constants'])
    evidence['5_no_oracle']=no_oracle_gate()
    evidence['2_attack_arithmetic']=attack_arithmetic_gate()
    jobs=read(P+'plan.json')['gate_jobs']
    dispatch(jobs[:2],'gate_initial')
    evidence['1_pre_onset']=compare_fields('gate_C_OFF','gate_A3_OFF',50)
    ...
    evidence['3_composition']=[compare_fields('gate_C_OFF','gate_C_NORMAL'),
                               compare_fields('gate_A3_OFF','gate_A3_NORMAL')]
    evidence['4_online_equivalence']={}
    for name in ('gate_C_GRADED','gate_A3_GRADED'):
        result=read(P+name+'_result.json')
        evidence['4_online_equivalence'][name]=result['offline_equivalence']
    result=read(P+'gate_A3_GRADED_result.json')
    evidence['6_incumbent_calls']={'passed':all(n==1 for n in result['incumbent_call_counts'].values()),
                                  'per_step_counts':result['incumbent_call_counts']}
    assert evidence['6_incumbent_calls']['passed']
    evidence['8_source_pins_end']=pins(full=True)
...
def scheduler_checks():
    ...
    assert len(pending)==78
    assert all(j['seed'] in range(1835087900,1835087920) for j in pending)
    ...
    result={'passed':True,'normal_cap':15,'work_cap':12,'slot_measurements':measured,
            'synthetic_resume_preserved':2,'synthetic_resume_pending':78,
...
def main():
    ...
    try:
        ...
        print(json.dumps({'status':'COMPLETE','runs':80,'report':P+'report.md','results':P+'results.json','manifest':P+'manifest.json'}),flush=True)
