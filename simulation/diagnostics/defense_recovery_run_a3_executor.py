"""Execute the registered recovery evaluation under the attempt 3 write guard."""
import sys
...
def allowed(path):
    p=Path(path).resolve()
    return p==NULL or(p.parent==OUT and p.name.startswith('defense_recovery_run_a3_'))
...
ARMS=('A3','H','M1')
DEFENSE_ARMS=('LATCHED','OFF','RECOVER-10','RECOVER-20','RECOVER-40')
...
def recovery_inertness_gate():
    left=read(P+'gate_H_LATCHED_result.json');right=read(P+'gate_H_RECOVER40_result.json')
    ...
    for x,y in zip(a,b):
        for key in x:
            ...
            if x[key]!=y[key] or type(x[key]) is not type(y[key]):
                first={'step':x['step'],'field':key,'latched':x[key],'recover_40':y[key]}
                break
        if first:break
    state_difference=next(({'step':x['step'],'latched':x['state_after_step'],'recover_40':y['state_after_step']}
...
def scheduler_checks():
    ...
    assert len(pending)==298
    assert all(j['seed'] in range(1835088000,1835088020) for j in pending)
    ...
    result={'passed':True,'normal_cap':15,'work_cap':12,'slot_measurements':measured,
            'synthetic_resume_preserved':2,'synthetic_resume_pending':298,
...
def main():
    ...
    try:
        ...
        print(json.dumps({'status':'COMPLETE','runs':300,'report':P+'report.md','results':P+'results.json','manifest':P+'manifest.json'}),flush=True)
