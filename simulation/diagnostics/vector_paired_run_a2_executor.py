...
def allowed(path):
    p=Path(path).resolve()
    return p==NULL or(p.parent==OUT and p.name.startswith('vector_paired_run_a2_'))
...
def no_corruption(self,model_state):
    return None
...
def scheduler_checks():
    assert dispatch_slots('normal',0,240)==15
    assert dispatch_slots('work',15,225)==0
    assert dispatch_slots('work',13,225)==0
    assert dispatch_slots('work',12,225)==0
    assert dispatch_slots('work',11,225)==1
    assert dispatch_slots('normal',12,225)==3
    ...
    assert len(pending)==238 and complete.isdisjoint(pending)
    assert set(pending)|complete==set(ids)
    for arm in read(P+'plan.json')['arms']:
        assert [s for a,s in jobs if a==arm]==list(range(1835087000,1835087060))
    return {'passed':True,'normal_cap':15,'work_cap':12,
            ...
            'synthetic_resume_pending':238}

def gates():
    import numpy as np
    import agents,run_attack_vector_revalidation_v2 as runner
    import inspect
    check_identity();source_start=pins()
    original=agents.AIAgent.apply_measurement_corruption
    assert inspect.signature(original)==inspect.signature(no_corruption)
    fake=object.__new__(agents.AIAgent)
    fake.measurement_tampering_active=True
    fake.policy='falsify_yield'
    np.random.seed(20260917)
    cases=[]
    for i in range(24):
        state={'avg_well_being':0.1+0.01*i,'population':100+i,
               'nested':{'unchanged':[i,True,None]},'theta_capability':0.3}
        if i%3==1:state['h_n_history']=[]
        elif i%3==2:state['h_n_history']=[0.125,0.2+0.01*i]
        expected=deepcopy(state)
        try:
            agents.AIAgent.apply_measurement_corruption=no_corruption
            rng=np.random.get_state()
            result=fake.apply_measurement_corruption(state)
            after=np.random.get_state()
            assert result is None and state==expected
            assert rng[0]==after[0] and np.array_equal(rng[1],after[1]) and rng[2:]==after[2:]
        finally:agents.AIAgent.apply_measurement_corruption=original
        assert agents.AIAgent.apply_measurement_corruption is original
        active=deepcopy(expected)
        fake.apply_measurement_corruption(active)
        assert active['avg_well_being']!=expected['avg_well_being']
        if expected.get('h_n_history'):
            assert active['h_n_history'][-1]!=expected['h_n_history'][-1]
        cases.append({'index':i,'before':expected,'control_after':state,
                      'production_after':active,'exact_control_equality':True,
                      'production_well_being_changed':True,
                      'production_last_entropy_changed':bool(expected.get('h_n_history')),
    ...
    for defense in (False,True):
        direct=task_for(1835087000,defense)
        ...
        assert m1.cop_independent_eval is defense and m1.random_seed==1835087000
    ...
    result={'passed':True,'utc':now(),'inertness_numpy_seed':20260917,
            'inertness_cases':cases,'construction':construction,
    ...
    print(json.dumps({'gates':'PASSED','synthetic_states':24,'construction_cells':2}),flush=True)
...
def run_job(arm,seed,attempt):
    import numpy as np
    import metrics,agents,run_attack_vector_revalidation_v2 as runner
    ...
    original_method=agents.AIAgent.apply_measurement_corruption
    ...
    active_method=original_method if arm['corruption_active'] else no_corruption
    with (OUT/partial).open('x',encoding='utf-8',newline='') as handle:
        ...
        def observed_factory(received):
            ...
            def observed_step():
                ...
                assert agents.AIAgent.apply_measurement_corruption is active_method
        ...
        try:
            agents.AIAgent.apply_measurement_corruption=active_method
            runner._make_model=observed_factory
            row=runner.run_single(task)
        finally:
            runner._make_model=original_factory
            agents.AIAgent.apply_measurement_corruption=original_method
        assert runner._make_model is original_factory
        assert agents.AIAgent.apply_measurement_corruption is original_method
    ...
    row.update(corruption_active=arm['corruption_active'],end_reason=end_reason,
    ...
    result={'status':'COMPLETE','job':job,'arm':arm,'seed':seed,'attempt':attempt,
            ...
            'production_method_restored':True,'continuous_checks_passed':True,
...
def batch():
    ...
    while pending or active:
        ...
        if time.monotonic()-last_print>=30:
            print(json.dumps({k:status[k] for k in ('state','completed','running','pending','mode','peak_active_workers')}),flush=True)
    ...
    print(json.dumps({'state':'COMPLETE','completed':finished,'peak_active_workers':peak}),flush=True)

def main():
    parser=argparse.ArgumentParser()
    parser.add_argument('command',choices=['gates','batch','worker'])
