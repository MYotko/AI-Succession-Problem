...
def allowed(path):
    p=Path(path).resolve()
    return p==NULL or(p.parent==OUT and p.name.startswith('detector_run_r2_eval_'))
...
def pins():
    plan=read('detector_run_r2_eval_plan.json');records=[]
...
def run_model(job,arm,seed,gate=False,attempt=1):
    ...
    if gate:result['datacollector']=model.datacollector
        ...
        output,detector_summary=evaluate_records(log_rows(final))
        result.update(detector_summary)
        hb=P+job+'_heartbeats_attempt'+str(attempt)+'.csv'
        alarms=P+job+'_alarms_attempt'+str(attempt)+'.csv'
        csv_output(hb,output['heartbeats'],['variant','variant_label','record_type','step','heartbeat_counter','S_H','S_g','S_L'])
        csv_output(alarms,output['alarm_records'],['variant','variant_label','record_type','step','channel','statistic_before_reset','statistic_after_reset','operational_channel'])
        spans=P+job+'_spans_attempt'+str(attempt)+'.csv'
        csv_output(spans,output['span_records'],['variant','variant_label','g_star_case','g_star_label','g_star','k','start_step','end_step','section_4_caveat'])
...
def compare_gates():
    ...
    results['derivation_conformance']=read(P+'derivation_gate.json')
    results['passed']=all(results['gate_'+str(i)]['passed'] for i in range(2,7)) and results['recorder_conformance']['passed'] and results['constants_conformance']['passed'] and results['derivation_conformance']['passed']
...
def gates():
    ...
    try:
        ...
        print(json.dumps({'T1':'PASS','gate_3':result['gate_3'],'recorder_conformance':result['recorder_conformance']['passed'],'constants_conformance':result['constants_conformance']['passed'],'derivation_conformance':result['derivation_conformance']['passed']}),flush=True)
    except BaseException as e:
        mark_halt('gates',e)
    ...
    try:
        while pending or active:
            ...
            for unused in range(dispatch_slots(mode,len(active),len(pending))):
                ...
                attempt=state['attempts'].get(job,0)+1;state['attempts'][job]=attempt
...
def constants():
    round1=json.loads(pinned_blob('simulation/diagnostics/detector_run_cal_constants.json'))['channels']
    stage_a=json.loads(pinned_blob('simulation/diagnostics/detector_run_r2_constants.json'))
    checks=[];variants={};hazards={}
    bits=lambda value:float(value).hex()
    threshold_pins={'T975':4.507729894543943,'T95':4.055050806319135,'T90':2.6499927544530903}
    for variant,wanted in threshold_pins.items():
        supplied={c:{key:round1[c][key] for key in ('reference','allowance','threshold')} for c in ('entropy','g','L')}
        supplied['g']['threshold']=stage_a['g_threshold_variants'][variant]['threshold']
        for channel in supplied:
            for key,value in supplied[channel].items():
                expected=stage_a['g_threshold_variants'][variant]['threshold'] if channel=='g' and key=='threshold' else stage_a['round1_parameters_carried_forward'][channel][key]
                checks.append({'variant':variant,'variant_label':'PRIMARY' if variant=='T975' else 'SECONDARY',
                               'channel':channel,'field':key,'supplied_binary64_hex':bits(value),
                               'committed_binary64_hex':bits(expected),'passed':bits(value)==bits(expected)})
        checks.append({'variant':variant,'field':'threshold pin','supplied_binary64_hex':bits(supplied['g']['threshold']),
                       'pinned_binary64_hex':bits(wanted),'passed':bits(supplied['g']['threshold'])==bits(wanted)})
        variants[variant]=supplied
    pins_k={'PRIMARY_2_0':(1.0965735902799727,9),'SECONDARY_1_5':(1.270310072072110,2),'SECONDARY_2_5':(0.966516292749662,106)}
    for label,(star,k) in pins_k.items():
        committed=stage_a['sustained_crossing_k'][label]
        hazard={'label':'PRIMARY' if label.startswith('PRIMARY') else 'SECONDARY','g_star':committed['g_star'],'k':committed['k']}
        check={'case':label,'stage_A_g_star_binary64_hex':bits(hazard['g_star']),'pinned_g_star_binary64_hex':bits(star),
               'stage_A_k':hazard['k'],'pinned_k':k,'passed':bits(hazard['g_star'])==bits(star) and hazard['k']==k}
        if label=='SECONDARY_2_5':hazard['section_4_caveat']=CAVEAT;check['section_4_caveat']=CAVEAT
        hazards[label]=hazard;checks.append(check)
    if not all(x['passed'] for x in checks):raise RuntimeError('Constants conformance failed')
    return variants,hazards,checks

def constants_gate():
    variants,hazards,checks=constants()
    result={'passed':all(x['passed'] for x in checks),'variants_passed':variants,'hazards_passed':hazards,
            'checks':checks,'identity':identity(),'utc':now(),'no_constants_recomputed_for_use':True}
    write(P+'constants_gate.json',result);return result
...
def derivation_gate():
    ...
    variants,hazards,checks=constants()
    stage_a=json.loads(pinned_blob('simulation/diagnostics/detector_run_r2_constants.json'))
    commit=stage_a['calibration_commit'];inputs=[];spans={label:[] for label in hazards}
    for seed in range(1835086300,1835086420):
        path='simulation/diagnostics/detector_run_cal_H_'+str(seed)+'_complete.json'
        raw=git('cat-file','blob',commit+':'+path)
        inputs.append({'path':path,'commit':commit,'sha256_lf':sha(lf(raw)),
                       'blob_sha1':hashlib.sha1(b'blob '+str(len(raw)).encode()+b'\0'+raw).hexdigest()})
        completion=json.loads(raw)
        path='simulation/diagnostics/'+completion['raw_log'];raw=git('cat-file','blob',commit+':'+path)
        measured=sha(lf(raw));expected=completion['raw_log_sha256_lf']
        inputs.append({'path':path,'commit':commit,'sha256_lf':measured,'completion_record_sha256_lf':expected,
                       'blob_sha1':hashlib.sha1(b'blob '+str(len(raw)).encode()+b'\0'+raw).hexdigest()})
        if measured!=expected:raise RuntimeError('Derivation gate input hash mismatch: '+path)
        rows=list(csv.DictReader(io.StringIO(raw.decode('utf-8'))))
        for label,values in hazards.items():
            longest=max((s['end_step']-s['start_step']+1 for s in spans_above(rows,values['g_star'])),default=0)
            spans[label].append({'seed':seed,'longest_span':longest})
    results={}
    for label,values in hazards.items():
        samples=[s['longest_span'] for s in spans[label]]
        percentile=float(np.percentile(samples,97.5,method='linear'));check_k=max(2,math.ceil(percentile))
        results[label]={**values,'derived_for_confirmation_only_k':check_k,'percentile_input_P':percentile,
                        'passed':check_k==values['k'],'per_run_longest_spans':spans[label]}
    result={'passed':all(r['passed'] for r in results.values()),'cases':results,'inputs':inputs,
            'no_value_replaced':True,'no_threshold_recomputed':True,'identity':identity(),'utc':now()}
    write(P+'derivation_gate.json',result)
    if not result['passed']:raise RuntimeError('Derivation conformance failed')
    return result

def evaluate_records(rows):
    import cusum_detector_v2 as detector
    variants,hazards,checks=constants();all_heartbeats=[];all_alarms=[];summaries={}
    for variant,supplied in variants.items():
        output=detector.detect(rows,supplied)
        if len(output['heartbeats'])!=len(rows):raise RuntimeError('Heartbeat count differs from completed steps: '+variant)
        first={c:next((r['step'] for r in output['alarm_records'] if r['channel']==c and r['step']>=50),None) for c in supplied}
        detection=next((step for step in output['operational_alarm_steps'] if step>=50),None)
        label='PRIMARY' if variant=='T975' else 'SECONDARY'
        all_heartbeats.extend({'variant':variant,'variant_label':label,**r} for r in output['heartbeats'])
        alarms=[{'variant':variant,'variant_label':label,**r} for r in output['alarm_records']]
        all_alarms.extend(alarms)
        summaries[variant]={'variant_label':label,'heartbeat_count':len(output['heartbeats']),
                            'alarm_records':alarms,'detection_alarm_step':detection,'first_detection_steps':first,
                            'constants_passed':supplied}
    recorded_hazards=hazard_records(rows,hazards)
    span_rows=[]
    for variant in variants:
        for label,values in recorded_hazards.items():
            for span in values['spans']:
                span_rows.append({'variant':variant,'variant_label':'PRIMARY' if variant=='T975' else 'SECONDARY',
                                  'g_star_case':label,'g_star_label':values['label'],'g_star':values['g_star'],
                                  'k':values['k'],**span,'section_4_caveat':values.get('section_4_caveat','')})
    return {'heartbeats':all_heartbeats,'alarm_records':all_alarms,'span_records':span_rows}, {
        'heartbeat_count':{v:r['heartbeat_count'] for v,r in summaries.items()},'variants':summaries,
        'hazards':recorded_hazards,'constants_bitwise_checks':checks}
