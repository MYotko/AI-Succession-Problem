...
import defense_heldout_run_a5_executor as ex
...
def report_complete(results,runs):
    lines=['# Held-out attacks, attempt 5','',
...
def finalize(results,lines,summaries):
    ...
    if (OUT/(P+'gates.json')).exists():
        ...
        for key,value in evidence.items():
            if key in ('source_pins_start','9_source_pins_end','runtime','2_attack_arithmetic','2_attack_effect'):continue
...
def main():
    if '--halt' in sys.argv:
        ...
        lines=['# Held-out attacks, attempt 5: halt','',
               'This is gate 2 of the promotion plan. The attacks were specified before any run.',
        ...
        for name in ('constants_gate.json','no_oracle_gate.json','gates.json','attack_arithmetic_gate.json','attack_effect_gate.json','burst_gate.json'):
