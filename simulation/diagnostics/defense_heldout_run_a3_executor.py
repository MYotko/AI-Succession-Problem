...
def allowed(path):
    p=Path(path).resolve()
    return p==NULL or(p.parent==OUT and p.name.startswith('defense_heldout_run_a3_'))
...
def attack_effect_gate():
    ...
    calls={c['step']:c for c in read(b['audit_file'])['attack_calls']}
    ...
    for row in attacked:
        ...
        if step>=len(control):
            ...
            continue
        c=control[step];call=calls[step]
        lower=row['x_resilience']<c['x_resilience']
        if step>50:below+=int(lower)
        for field in FIELDS:
            if field=='x_resilience':continue
            delta=abs(row[field]-c[field])
            bound=call['moved']+call['normalization_absolute_changes'].get(field,0.0)
            comparisons.append({'step':step,'field':field,'absolute_control_difference':delta,'bound':bound})
            if delta>bound and difference is None:
                difference={'step':step,'field':field,'control':c[field],'attacked':row[field],
                            'absolute_difference':delta,'siphoned_amount':call['moved'],
                            'normalization_absolute_change':call['normalization_absolute_changes'].get(field,0.0),
                            'bound':bound}
...
def gates():
    ...
    for step in (50,51,52):
        ...
        burst_cases.append({'step':step,'actual':actual,'expected_production_hostile_action':expected,'passed':actual==expected and call['kind']=='burst'})
