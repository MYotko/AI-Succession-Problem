"""Execute detector calibration and its required gates under a write guard."""
import sys
...
def allowed(path):
    p=Path(path).resolve()
    return p==NULL or p==ROOT/'simulation/cusum_detector_v2.py' or(p.parent==OUT and p.name.startswith('detector_run_cal_'))
...
def pins():
    plan=read('detector_run_cal_plan.json');records=[]
...
def run_model(job,arm,seed,gate=False,attempt=1):
    ...
    if not gate and (arm!='H' or model.random_seed!=seed or seed not in plan['seeds']):
        raise RuntimeError('Calibration must use the specified honest seed and arm')
...
def scheduler_checks():
    ...
    assert len(jobs)==120 and len(set(jobs))==120 and len(pending)==118
