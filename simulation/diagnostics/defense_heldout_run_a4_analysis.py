...
import defense_heldout_run_a4_executor as ex
...
def report_complete(results,runs):
    lines=['# Held-out attacks, attempt 4','',
       ...
       'Attempt 2 halted before any model step on an incorrect scheduler upper seed bound, corrected in attempt 3. Attempt 3 halted at T1 gate 2 after two gate runs and zero batch runs. Attempt 4 applies the dispatched floating-point gate tolerances.','']
...
def main():
    if '--halt' in sys.argv:
        ...
        lines=['# Held-out attacks, attempt 4: halt','',
               'This is gate 2 of the promotion plan. The attacks were specified before any run.',
